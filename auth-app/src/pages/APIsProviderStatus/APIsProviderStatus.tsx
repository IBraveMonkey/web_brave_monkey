...
const APIsProviderStatus: React.FC = () => {
  ...
  useEffect(() => {
    const fetchHealth = async () => {
      try {
        ...
        setHealthData(response as unknown as HealthStatus);
      } catch (err) {
  ...
  return (
    <div className={styles.container}>
            ...
            <div className={styles.providersList}>
              {category.providers.map((provider, providerIndex) => (
                ...
                >
                  ...
                  <div className={`${styles.statusIndicator} ${styles[getStatus(provider.key)]}`}>
                    {getStatus(provider.key) === 'active' ? t('apiStatus.active') : t('apiStatus.inactive')}
                  </div>
                </div>
              ))}
            </div>
      ...
      {loading && <p style={{ textAlign: 'center', marginTop: 10 }}>Refreshing status...</p>}
    </div>
  );
};
